import { View, StatusBar, ScrollView, TouchableOpacity, Image, Text, FlatList } from 'react-native';
import React, { useEffect } from 'react';
import COLORS from 'values/colors';
import Svg from 'atoms/Svg';
import { styles } from './styles';
import { h } from 'values/Dimensions';
import Header from './Components/Head/Header';
import Button from 'components/molecules/ButtonIcon';
import { useNavigation } from '@react-navigation/native';
import { useSelector } from 'react-redux';
import User, { selectHomeData } from 'redux/user';
import { useAppDispatch } from 'redux/store';
import thunks from 'redux/user/thunks';
import { TransferDate } from 'src/HF/HF';
import { selectCurrentUser } from 'redux/auth';

const DashboardScreen = () => {
  const { navigate } = useNavigation<any>()
  const navigation = useNavigation<any>()
  const dispatch = useAppDispatch()
  const uri = 'data:image/jpeg;base64,'
  const HomeData = useSelector(selectHomeData)
  const CurrentUser = useSelector(selectCurrentUser)

  // alert(JSON.stringify(CurrentUser))

  useEffect(() => {
    const RenderFunction = navigation.addListener('focus', () => {
      dispatch(thunks.getHome())
      dispatch(User.changeAccepted(false))
      dispatch(User.changeRejected(false))
      dispatch(User.changeInvited(false))
      dispatch(User.changeUploaded(false))
    })
    return RenderFunction 
  }, [navigation])
  return (
    <View
      style={{
        height: h * 1,
        justifyContent: 'flex-start',
        backgroundColor: COLORS.bgDWhite,
      }}>
      <StatusBar backgroundColor={COLORS.primary} barStyle="dark-content" />
      <Header />
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.container}>
          <TouchableOpacity
            style={styles.EditButton}
            onPress={() => { }}>
            <Svg name="edit" />
          </TouchableOpacity>
          <Image style={styles.Image} source={{ uri: uri }} />
          <Text style={styles.Name}>{HomeData?.user?.name}</Text>

          <TouchableOpacity onPress={() => { navigate('RecentDisputes') }} activeOpacity={.8} style={styles.InvitationsContainer}>
            <Text style={styles.InvitationsText}>Work Invitations</Text>
          </TouchableOpacity>

          <View style={styles.Card}>
            <Text style={styles.WalletBalanceText}>Wallet Balance</Text>
            <View style={styles.RowBetween}>
              <Text style={styles.salaryText}>${HomeData?.user?.wallet}</Text>
              <TouchableOpacity onPress={() => navigate('Wallet')}>
                <Svg name="wallet" style={{ marginRight: 12 }} />
              </TouchableOpacity>
            </View>
            <Text style={styles.smallText}>All Escrows*</Text>
          </View>

          <View style={styles.Card}>
            <Text style={styles.WalletBalanceText}>Escrows</Text>
            <Text style={styles.smallHeaderText}>As a Seller</Text>

            <FlatList
              data={HomeData?.as_supplier}
              nestedScrollEnabled
              contentContainerStyle={{ marginBottom: 10 }}
              ListEmptyComponent={() => <Svg name='NoEscrow' style={{ alignSelf: 'center', marginTop: 25 }} />}
              renderItem={({ item }) => (
                <TouchableOpacity activeOpacity={.8} onPress={() => navigate('Escrow', { type: 'profile', id: item?.id })} style={[styles.SupplierCard]}>
                  <Image style={styles.S_Image} source={{ uri: uri }} />
                  <View style={[styles.TextContainer, { height: 50, }]}>
                    <Text style={styles.smallHeaderText}>{item?.name}</Text>
                    <Text style={styles.smallText}>{item.description}</Text>
                    <Text style={styles.smallText}>Deadline<Text style={{ color: COLORS.primary }}> {TransferDate(item?.start_date)}</Text></Text>
                  </View>
                  <Svg name="circleArrow" />
                </TouchableOpacity>
              )}
            />

          </View>

          <View style={[styles.Card, { marginBottom: 40 }]}>
            <Text style={styles.smallHeaderText}>As a Buyer</Text>
            <FlatList
              data={HomeData?.as_buyer}
              nestedScrollEnabled
              contentContainerStyle={{ marginBottom: 10 }}
              ListEmptyComponent={() => <Svg name='NoEscrow' style={{ alignSelf: 'center', marginTop: 25 }} />}
              renderItem={({ item }) => (
                <>
                  {item.status == 1 &&
                    <TouchableOpacity activeOpacity={.8} onPress={() => navigate('Escrow', { type: 'buyer', id: item?.id })} style={[styles.SupplierCard]}>
                      <Image style={styles.S_Image} source={{ uri: uri }} />
                      <View style={[styles.TextContainer, { height: 35, }]}>
                        <Text style={styles.smallHeaderText}>{item?.name}</Text>
                        <Text style={styles.smallText}>{item.description}</Text>
                      </View>
                      <Svg name="circleArrow" />
                    </TouchableOpacity>}
                </>
              )}
            />

            <Button
              svg='add'
              style={{ marginTop: h * 0.016 }}
              type="primary"
              label="Create Escrow"
              onPress={() => {
                navigate('CreatePackage')
              }}
            />
          </View>

        </View>
      </ScrollView>
    </View>
  );
};

export default DashboardScreen;
