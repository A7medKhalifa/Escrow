    ...
    <View
      ...
      <ScrollView nestedScrollEnabled showsVerticalScrollIndicator={false}>
        <View style={styles.container}>
          <Text style={[styles.Head, { marginTop: 10 }]}>Deposit Fund</Text>

          <Text style={styles.WalletBalanceText}>Reference Number</Text>
          <Text style={styles.BigText}>#5345</Text>

          <Text style={styles.WalletBalanceText}>Escrow Value</Text>
          <Text style={styles.BigText}>750.00 US$</Text>

          <Text style={[styles.WalletBalanceText, { marginTop: 30 }]}>Choose Payment method</Text>

          <FlatList
            data={[
              { type: 'Credit', name: 'Credit Card' },
              { type: 'Visa', name: 'Visa Card' },
              { type: 'Master', name: 'Master Card' },
              { type: 'BankTranfer', name: 'Bank Transfer' },
            ]}
            renderItem={({ item }) =>
              <View style={styles.methodContainer}>
                <Svg name={item.type} style={{ alignSelf: 'center' }} />
                <Text style={[styles.Head, { fontSize: 12, marginLeft: 15 }]}>{item.name}</Text>
              </View>
            }
          />
          <Button
