import { View, StatusBar, ScrollView, Image, Text, FlatList, TouchableOpacity } from 'react-native';
import React, { useEffect } from 'react';
import COLORS from 'values/colors';
import { styles } from './styles';
import { h } from 'values/Dimensions';
import Header from './Components/Head/Header';
import Svg from 'atoms/Svg';
import { useNavigation, useRoute } from '@react-navigation/native';
import { Formik } from 'formik';
import { InviteSupplier_initial_values, } from 'src/formik/initial_values';
import { InviteSupplierScheme } from 'src/formik/schema';
import InputView from 'components/molecules/Input';
import Button from 'components/molecules/shortbutton';
import { useAppDispatch } from 'redux/store';
import thunks from 'redux/user/thunks';
import { useSelector } from 'react-redux';
import { selectAllUsers, selectInvited } from 'redux/user';

const InviteSupplierScreen = () => {
  const uri = 'data:image/jpeg;base64,'
  const { navigate, goBack } = useNavigation<any>()
  const { id }: any = useRoute().params;
  const dispatch = useAppDispatch()
  const AllUsers = useSelector(selectAllUsers)
  const DoneInvited = useSelector(selectInvited)

  useEffect(() => {
    dispatch(thunks.getUsers())
  }, [])
  // console.log(AllUsers)

  useEffect(() => {
    DoneInvited && navigate('Dashboard')
  }, [DoneInvited])

  return (
    <View
      style={{
        height: h * 1,
        justifyContent: 'flex-start',
        backgroundColor: COLORS.bgDWhite,
      }}>
      <StatusBar backgroundColor={COLORS.primary} barStyle="dark-content" />
      <Header />
      <ScrollView nestedScrollEnabled showsVerticalScrollIndicator={false}>
        <View style={styles.container}>

          <View style={styles.Card}>
            <Text style={styles.WalletBalanceText}>Invite seller</Text>
            <Text style={[styles.WalletBalanceText, { color: COLORS.DDarkBlue, fontWeight: '500' }]}>Seller Contacts</Text>
            <Formik
              initialValues={InviteSupplier_initial_values}
              validationSchema={InviteSupplierScheme}
              onSubmit={values => {
                // navigate('Expenditures')
              }}>{(props =>
                <>
                  <InputView
                    {...props}
                    placeholder="Phone number"
                    values={props.values}
                    name="mobile"
                    keyboardType="numeric"
                  />
                  <InputView
                    {...props}
                    placeholder="Email"
                    values={props.values}
                    name="email"
                  />
                </>
              )}


            </Formik>
            <Button
              style={{ marginTop: 15, width: '90%', alignSelf: 'center' }}
              size={20}
              type="primary"
              label="Invite seller"
              onPress={() => {
                dispatch(thunks.inviteToEscrow({
                  user_id: '',
                  escrow_id: id
                }))
              }}
            />
          </View>
          <Text style={[styles.smallText, { marginTop: 18, marginLeft: 20 }]}>Suggestions</Text>
          <View style={styles.Card}>
            <FlatList
              data={AllUsers}
              ListEmptyComponent={() => <Svg name='NoUsers' style={{ alignSelf: 'center' }} />}
              renderItem={({ item }) => (
                <TouchableOpacity onPress={() => {
                  dispatch(thunks.inviteToEscrow({
                    user_id: item?.id,
                    escrow_id: id
                  }))
                }} style={[styles.SupplierCard]}>
                  {/* {console.warn(item?.id)} */}
                  <Image style={styles.S_Image} source={{ uri: uri }} />
                  <Text style={styles.smallHeaderText}>{item?.name}</Text>
                  <View style={{ width: 40 }} />
                </TouchableOpacity>
              )}
            />



          </View>


        </View>
      </ScrollView>
    </View>
  );
};

export default InviteSupplierScreen;
