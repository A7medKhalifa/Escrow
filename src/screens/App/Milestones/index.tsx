...
import { selectEscrowDetails, selectInvitationEscrow } from 'redux/user';
      ...
      <ScrollView nestedScrollEnabled showsVerticalScrollIndicator={false}>
        <View style={styles.container}>
          ...
          <View style={[styles.Card, { alignItems: 'center' }]}>
            <Text style={styles.CardHeader}>Escrow Bchain App Development</Text>
            <Image style={styles.Image} source={{ uri: uri }} />
            <Text style={styles.smallText}>Buyer: {EscrowDetails?.buyer_name}</Text>
          </View>
          ...
          <View style={[styles.Card, { marginBottom: 30 }]}>
            <Text style={styles.WalletBalanceText}>Milestones</Text>
            <FlatList
              data={EscrowDetails?.milestones}
              renderItem={({ item }) => (
                <>
                  <View style={[styles.RowBetween, { marginTop: 15, marginBottom: 5, }]}>
                    <View style={styles.ProgressCircleBorder}>
                      {item?.status == 1 && <View style={styles.ProgressCircle} />}
                    </View>

                    <TouchableOpacity activeOpacity={.8}
                      onPress={() => { navigate('MilestonesType', { id: item.id }) }}
                      style={styles.SupplierCard}>
                      <Image style={styles.S_Image} source={{ uri: uri }} />

                      <View style={{ marginLeft: 10, width: '80%' }}>
                        <View style={styles.TextContainer}>
                          <Text style={[styles.WalletBalanceText, { fontSize: 15, maxWidth: '65%' }]}>{item?.name}</Text>
