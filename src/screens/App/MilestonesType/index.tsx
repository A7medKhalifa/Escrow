import { View, StatusBar, ScrollView, Image, Text, FlatList } from 'react-native';
import React, { useEffect, useState } from 'react';
import COLORS from 'values/colors';
import { styles } from './styles';
import { h } from 'values/Dimensions';
import Header from './Components/Head/Header';
import Svg from 'atoms/Svg';
import Button from 'components/molecules/shortbutton';
import DoneModal from './Components/DoneModal/Done';
import { TouchableOpacity } from 'react-native-gesture-handler';
import DeleteModal from './Components/DeleteModal/Delete';
import ChangeModal from './Components/ChangeModal/Change';
import CompleteModal from './Components/CompleteModal/Complete';
import { useNavigation, useRoute } from '@react-navigation/native';
import SuccessModal from './Components/SuccessModal/SuccessModal';
import { useSelector } from 'react-redux';
import { selectMilestone, selectUploaded } from 'redux/user';
import { useAppDispatch } from 'redux/store';
import thunks from 'redux/user/thunks';
import { TransferDate } from 'src/HF/HF';
import useLibraryPermission from 'hooks/useLibraryPermission';


const MilestonesTypeScreen = () => {
  const uri = 'data:image/jpeg;base64,'
  const { navigate } = useNavigation<any>()
  const { id }: any = useRoute().params;
  const dispatch = useAppDispatch()
  const [show, setShow] = useState(false)
  const [showDelete, setShowDelete] = useState(false)
  const [ShowChange, setShowChange] = useState(false)
  const [ShowComplete, setShowComplete] = useState(false)
  const Milestone = useSelector(selectMilestone)
  const Uploaded = useSelector(selectUploaded)
  const { UploadFile } = useLibraryPermission()

  useEffect(() => {
    dispatch(thunks.getMilestone(id))
  }, [])

  useEffect(() => {
    Uploaded && setShow(true)
  }, [Uploaded])
  // { console.log(Milestone) }
  return (
    <View
      style={{
        height: h * 1,
        justifyContent: 'flex-start',
        backgroundColor: COLORS.bgDWhite,
      }}>
      <StatusBar backgroundColor={COLORS.primary} barStyle="dark-content" />
      <Header />
      <ScrollView nestedScrollEnabled showsVerticalScrollIndicator={false}>
        <View style={styles.container}>


          <View style={[styles.Card, { marginBottom: 30 }]}>
            <View style={{ flexDirection: 'row', width: '97%' }}>
              <Image style={styles.S_Image} source={{ uri: uri }} />
              <View style={{ marginLeft: 10, width: '87.5%' }}>
                <View style={styles.TextContainer}>
                  <Text style={[styles.WalletBalanceText, { fontSize: 15 }]}>{Milestone?.milestone?.name}</Text>
                  <Text style={[styles.WalletBalanceText, { fontWeight: '700' }]}>{Milestone?.milestone?.value + ' ' + Milestone?.milestone?.currency}</Text>
                </View>
                <Text style={styles.grayText}>Backend Development</Text>
                <Text style={styles.grayText}>Deadline: {TransferDate(Milestone?.milestone?.completion_date)}</Text>
                <Text style={[styles.grayText, { marginTop: 25, fontSize: 13 }]}>{Milestone?.milestone?.description}</Text>
              </View>
            </View>

            {/* <TouchableOpacity activeOpacity={.8} style={styles.UploadButton}
              onPress={() => navigate('UploadDocument')} >
              <Svg name='file' bgColor={COLORS.black} />
              <Text style={styles.UploadText}>Upload document</Text>
            </TouchableOpacity> */}
            {/* {alert(Milestone?.type)} */}
            {
              Milestone?.type != 'buyer' &&
              <Button
                style={{ marginTop: h * 0.04 }}
                type="primary"
                label="Upload document"
                onPress={() => {
                  UploadFile(id, dispatch)
                  // navigate('UploadDocument')
                }}
              />
            }

            {/* <DoneModal show={show} setShow={setShow} /> */}
            <DeleteModal setShow={setShowDelete} show={showDelete} />
            <ChangeModal setShow={setShowChange} show={ShowChange} />
            <CompleteModal setShow={setShowComplete} show={ShowComplete} />
            <SuccessModal show={show} setShow={setShow} />
          </View>


        </View>
      </ScrollView>
    </View>
  );
};

export default MilestonesTypeScreen;
