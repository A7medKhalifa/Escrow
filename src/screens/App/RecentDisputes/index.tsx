import { View, StatusBar, ScrollView, Image, Text, FlatList, TouchableOpacity } from 'react-native';
import React, { useEffect } from 'react';
import COLORS from 'values/colors';
import { styles } from './styles';
import { h } from 'values/Dimensions';
import Header from './Components/Head/Header';
import { useNavigation } from '@react-navigation/native';
import { useSelector } from 'react-redux';
import User, { selectReceivedInvitations } from 'redux/user';
import { useAppDispatch } from 'redux/store';
import thunks from 'redux/user/thunks';
import { TransferDate } from 'src/HF/HF';
import Svg from 'atoms/Svg';

const RecentDisputesScreen = () => {
  const uri = 'data:image/jpeg;base64,'
  const { navigate } = useNavigation<any>()
  const navigation = useNavigation<any>()
  const dispatch = useAppDispatch()
  const Invitations = useSelector(selectReceivedInvitations)

  useEffect(() => {
    const RenderFunction = navigation.addListener('focus', () => {
      dispatch(thunks.myReceivedInvitation(``))
      dispatch(User?.changeRejected(false))
      dispatch(User?.changeAccepted(false))
    })
    return RenderFunction
  }, [navigation])

  return (
    <View
      style={{
        height: h * 1,
        justifyContent: 'flex-start',
        backgroundColor: COLORS.bgDWhite,
      }}>
      <StatusBar backgroundColor={COLORS.primary} barStyle="dark-content" />
      <Header />
      <ScrollView nestedScrollEnabled showsVerticalScrollIndicator={false}>
        <View style={styles.container}>
          <Text style={styles.Head}>New Escrow Invitations</Text>
          <FlatList
            data={Invitations}
            nestedScrollEnabled
            style={{ marginBottom: 40 }}
            ListEmptyComponent={() => <Svg name='NoInvite' style={{ alignSelf: 'center', marginTop: 50 }} />}
            renderItem={({ item }) => (
              <TouchableOpacity activeOpacity={.8} onPress={() => {
                navigate('Escrow', { type: 'invitation', id: item.id })
              }} style={styles.Card}>
                <View style={styles.RowCenter}>
                  <View style={{}}>
                    <Image style={styles.S_Image} source={{ uri: uri }} />
                    <View style={styles.absoluteSmallCircle}><Text style={{ color: COLORS.white }}>O</Text>
                    </View>
                  </View>
                  <View style={styles.TextContainer}>
                    <Text style={styles.smallHeaderText}>{item?.escrow_code}</Text>
                    <Text style={styles.smallText}>{item?.escrow_name}</Text>
                    <Text style={styles.smallHeaderText}>Buyer</Text>
                    <Text style={styles.smallHeaderText}>{item?.sender_name}</Text>
                    <Text style={styles.smallText}>4 Milestones</Text>
                  </View>
                </View>
                <View style={styles.PackageContainer}>
                  <View style={styles.SmallCircle}>
                    <Text style={{ color: COLORS.white }}>O</Text>
                  </View>
                  <Text style={styles.PackageText}>Escrow Value : {item?.escrow_value}</Text>
                </View>
                <Text style={styles.DateText}>Received: {TransferDate(item?.escrow_start_date)}</Text>
              </TouchableOpacity>
            )}
          />


        </View>
      </ScrollView>
    </View>
  );
};

export default RecentDisputesScreen;
