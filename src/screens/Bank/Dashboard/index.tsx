import { View, StatusBar, ScrollView, TouchableOpacity, Image, Text, FlatList } from 'react-native';
import React from 'react';
import COLORS from 'values/colors';
import Svg from 'atoms/Svg';
import { styles } from './styles';
import { h } from 'values/Dimensions';
import Header from './Components/Head/Header';
import Button from 'components/molecules/ButtonIcon';
import { useNavigation } from '@react-navigation/native';

const DashboardScreen = () => {
  const { navigate } = useNavigation<any>()
  const uri = 'data:image/jpeg;base64,'

  return (
    <View
      style={{
        height: h * 1,
        justifyContent: 'flex-start',
        backgroundColor: COLORS.bgDWhite,
      }}>
      <StatusBar backgroundColor={COLORS.primary} barStyle="dark-content" />
      <Header />
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.container}>
          <TouchableOpacity
            style={styles.EditButton}
            onPress={() => { }}>
            <Svg name="edit" />
          </TouchableOpacity>
          <Image style={styles.Image} source={{ uri: uri }} />
          <Text style={styles.Name}>Bank</Text>

          <View>
            <TouchableOpacity onPress={() => { }} activeOpacity={.8} style={styles.InvitationsContainer}>
              <Text style={styles.InvitationsText}>Pending Payments</Text>
            </TouchableOpacity>
            <View style={{
              position: 'absolute',
              right: 0,
              top: -3,
              height: 20,
              width: 20,
              borderRadius: 20,
              backgroundColor: COLORS.sky,
              justifyContent: 'center',
              alignItems: 'center'
            }}>
              <Text>3</Text>
            </View>
          </View>


          {/* <View style={styles.Card}>
            <Text style={styles.WalletBalanceText}>Wallet Balance</Text>
            <View style={styles.RowBetween}>
              <Text style={styles.salaryText}>$1,250.00</Text>
              <TouchableOpacity onPress={() => navigate('Wallet')}>
                <Svg name="wallet" style={{ marginRight: 12 }} />
              </TouchableOpacity>
            </View>
            <Text style={styles.smallText}>All Escrows*</Text>
          </View> */}

          <View style={styles.Card}>
            <Text style={[styles.WalletBalanceText,{fontWeight:'600'}]}>Withdraw Requests</Text>

            <FlatList
              data={[{}, {}, {}]}
              nestedScrollEnabled
              contentContainerStyle={{ marginBottom: 10 }}
              renderItem={() => (
                <TouchableOpacity activeOpacity={.8} onPress={() => navigate('AquaticsApplication')} style={[styles.SupplierCard]}>
                  <Image style={styles.S_Image} source={{ uri: uri }} />
                  <View style={[styles.TextContainer, { height: 50, }]}>
                    <Text style={styles.smallHeaderText}>Aquatics</Text>
                    <Text style={styles.smallText}>App Development</Text>
                    <Text style={styles.smallText}>Deadline<Text style={{ color: COLORS.primary }}> March 10 </Text><Text style={{ color: COLORS.move }}> $1000.00</Text></Text>
                  </View>
                  <Svg name="circleArrow" />
                </TouchableOpacity>
              )}
            />

          </View>

          <View style={styles.Card}>
            <View style={{ justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center' }}>
              <Text style={[styles.WalletBalanceText,{fontWeight:'600'}]}>Deposit Funds Confirmation</Text>
              <Text style={[styles.WalletBalanceText, { fontSize: 10 }]}>(AUTOMATED)</Text>
            </View>

            <FlatList
              data={[{}, {}, {}]}
              nestedScrollEnabled
              contentContainerStyle={{ marginBottom: 10 }}
              renderItem={() => (
                <TouchableOpacity activeOpacity={.8} onPress={() => navigate('AquaticsApplicationFunds')} style={[styles.SupplierCard]}>
                  <Image style={styles.S_Image} source={{ uri: uri }} />
                  <View style={[styles.TextContainer, { height: 35, }]}>
                    <Text style={styles.smallHeaderText}>Mazera App Development</Text>
                    <Text style={styles.smallText}>Joey Tribiani</Text>
                  </View>
                  <Svg name="circleArrow" />
                </TouchableOpacity>
              )}
            />

            <Button
              svg='add'
              style={{ marginTop: h * 0.016 }}
              type="primary"
              label="Manual Transaction"
              onPress={() => {
                navigate('Home')
              }}
            />
          </View>

        </View>
      </ScrollView>
    </View>
  );
};

export default DashboardScreen;
