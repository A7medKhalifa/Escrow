import { View, StatusBar, ScrollView, Image, Text, FlatList, TouchableOpacity, Dimensions } from 'react-native';
import React from 'react';
import COLORS from 'values/colors';
import { styles } from './styles';
import { h } from 'values/Dimensions';
import Header from './Components/Head/Header';
import QRCode from 'react-native-qrcode-svg';
import Button from 'components/molecules/ButtonIcon';
import Barcode from '@kichiyaki/react-native-barcode-generator';
import { useRoute } from '@react-navigation/native';

const OwnerDetailsScreen = () => {
  const uri = 'data:image/jpeg;base64,'

  return (
    <View
      style={{
        height: h * 1,
        justifyContent: 'flex-start',
        backgroundColor: COLORS.bgDWhite,
      }}>
      <StatusBar backgroundColor={COLORS.primary} barStyle="dark-content" />
      <Header />
      <ScrollView nestedScrollEnabled showsVerticalScrollIndicator={false}>
        <View style={styles.container}>

          <Image style={styles.Image} source={{ uri: uri }} />
          <Text style={styles.Name}>Mina Ezzat</Text>

          {/* <View style={styles.Card}>
            <Text style={styles.WalletBalanceText}>Escrow Number</Text>
            <Barcode
              format="CODE128"
              value={'121247631943'}
              text={'121247631943'}
              textStyle={{ color: COLORS.black, alignSelf: 'flex-start', marginTop: 10 }}
              style={{ alignSelf: 'flex-start', marginTop: 10 }}
              maxWidth={Dimensions.get('window').width / 1.5}
              width={150}
              height={50}
            />
          </View> */}
          <View style={[styles.Card, { marginTop: 40 }]}>
            <Text style={[styles.smallText, { position: 'absolute',fontWeight:'700', top: -h * .035, left: 12.5 }]}>Buyer Info</Text>

            <Text style={[styles.WalletBalanceText, { fontWeight: 'bold' }]}>Mina Ezzat</Text>
            <Text style={styles.smallText2}>CEO at O-Projects</Text>
            <View style={styles.divider} />
            <Text style={styles.smallText}>Gender</Text>
            <Text style={styles.WalletBalanceText}>Male</Text>

            <Text style={styles.smallText}>Phone Number</Text>
            <Text style={styles.WalletBalanceText}>01273564321</Text>

            <Text style={styles.smallText}>Email</Text>
            <Text style={styles.WalletBalanceText}>Ahmedkhalifa20199@gmail.com</Text>

            <Text style={styles.smallText}>Address</Text>
            <Text style={styles.WalletBalanceText}>19 Doc Ahmed Zaki Street, Cairo, Egypt</Text>

            <Text style={styles.smallText}>National ID Number</Text>
            <Text style={styles.WalletBalanceText}>29207270101235</Text>
          </View>

          <View style={[styles.Card, { alignItems: 'center',marginBottom:30 }]}>
            <QRCode
              value="https://o-projects.org/"
              size={170}
            />
          </View>
        </View>
      </ScrollView>
    </View>
  );
};

export default OwnerDetailsScreen;
